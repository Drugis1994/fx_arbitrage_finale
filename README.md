...
## Testing / CI
- C unit tests: `cmake -S C++ -B build && cmake --build build && ctest --test-dir build` (matrix bounds/update guards).
- R unit tests: `Rscript -e "testthat::test_dir('tests/testthat')"`.
- GitHub Actions workflow (`ci.yml`) runs both suites on Linux/macOS.
